<template>
    <view class="item-main" :style="height ? 'height:' + height + 'rpx;' : ''">
        <view class="item-list">
            ...
            <scroll-view class="item-right" scroll-y="true" :scroll-with-animation="true" :scroll-into-view="scrollIntoView">
                <view class="item-right-list" v-for="(item, index) of scrollLeftObj" :key="index" :id="index != '#' ? index : 'BOTTOM'">
                    <view class="left-item-title" v-if="item && item.length">{{ index }}</view>
                    <view class="item-right-list-name" v-for="(mess, inx) in item" @click.stop="selectRight(mess)" :index="inx">
                        <view class="item-right-list-name-info" :style="mess.chooseState ? 'color:' + themeColor : ''">
                            {{ mess.name }}
                        </view>
                        <image v-if="mess.chooseState" class="item-right-list-img" src="../../static/select.png"> </image>
                    </view>
                </view>
            </scroll-view>
            <view class="liu-scroll-right">
                <view
                    :style="item == scrollIntoView ? 'background: ' + themeColor : ''"
