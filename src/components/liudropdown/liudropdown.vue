    ...
    <view>
        <view class="popup-box" :style="popupBox">
            <view class="type-list">
                <view
                    class="type-item"
                    :style="'width:' + 100 / newMenuList.length + '%'"
                    v-for="(item, index) in newMenuList"
                    :key="index"
                    @click.stop="selectType(index, item)"
                >
                    <view class="type-item-name" :style="selectIndex == index || item.showName ? 'color:' + themeColor : ''">
                        {{ item.showName || item.name }}
                    </view>
                    <view class="type-item-icon" :style="selectIndex == index || item.showName ? 'background-color:' + themeColor : ''">
            ...
            <view :catchtouchmove="isShow" class="scroll-popup" :style="scrollPopup" v-if="isShow">
                <view v-if="itemList.length > 0">
                    <item1 v-if="showType == 1" :itemList="itemList" :isMultiple="isMultiple" :themeColor="themeColor" @complete="complete"></item1>
                    <item2 v-if="showType == 2" :itemList="itemList" :isMultiple="isMultiple" :themeColor="themeColor" @complete="complete"></item2>
                    <item3 v-if="showType == 3" :itemList="itemList" :isMultiple="isMultiple" :themeColor="themeColor" @complete="complete"></item3>
                    <item4 v-if="showType == 4" :itemList="itemList" :isMultiple="isMultiple" :themeColor="themeColor" @complete="complete"></item4>
                </view>
...
export default {
    ...
    mounted() {
        ...
        document.querySelector('.scroll-mask').addEventListener(
            'touchmove',
            function (event) {
                event.preventDefault();
            },
    ...
    methods: {
        ...
        selectType(index, item) {
            ...
            if (this.selectIndex == index) {
                ...
                this.isMultiple =
                    this.selectIndex > -1 && this.menuList[this.selectIndex].isMultiple ? this.menuList[this.selectIndex].isMultiple : false;
